class Photo
  ...
  def populate_liked_commented_users(opts = {})
    ...
    lkd_usr_ids ||= self.likes.where(:user_id.nin => inactve_usr_ids).desc(:created_at).limit(5).only(:user_id).collect(&:user_id)
    cur_usr_id = lkd_usr_ids.delete(current_user.id)
    cmt_usr_ids ||= self.comments.where(:user_id.nin => inactve_usr_ids).desc(:created_at).limit(2).only(:user_id).collect(&:user_id)
